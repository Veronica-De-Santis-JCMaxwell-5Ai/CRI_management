...
<?php
	...
	$query = "SELECT COUNT(*) AS id FROM dialisi";
	...
	$query = "INSERT INTO dialisi VALUES($codD, '$dataTrasporto', $sigla, '$orario', '$trasportatoDa', '$trasportatoA', $durata, '$cf', $autista, $barrelliere1, $barrelliere2);";
	$result = mysqli_query($conn, $query)
		or die("Attenzione, errore ". mysqli_error($conn));
	
	$query = "SELECT COUNT(*) AS isThere FROM pazienti WHERE cf = '$cf';";
	$result = mysqli_query($conn, $query)
		or die("Attentione, errore ". mysqli_error($conn));
	$row = mysqli_fetch_array($result);
	if($row['isThere']==0)
	{
